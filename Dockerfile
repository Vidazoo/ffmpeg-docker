...
RUN apt-get install -y time yasm nasm \
                ...
                checkinstall
RUN apt-get install -y libmp3lame0  libmp3lame-dev
...
RUN wget https://www.ffmpeg.org/releases/ffmpeg-3.0.tar.bz2 &&\
                ...
                ./configure  --with-openssl--enable-libmp3lame --enable-libvorbis --enable-gpl --enable-libx264 --enable-libtheora --enable-libvpx  --prefix=/usr && time make -j 8 && mkdir /usr/share/ffmpeg && \
